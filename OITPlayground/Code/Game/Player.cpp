	...
	m_owner(owner)
{
	...
	m_camera.SetTransform(initialPosition, EulerAngles());
...
void Player::InputUpdate()
{
	...
	Vec3 jLeft;
	Vec3 kUp;
	EulerAngles currentOrientation = m_camera.GetCameraOrientation();
	...
	Vec3 currentPosition	= m_camera.GetCameraPosition();
	currentPosition			+= moveIntentions * m_speed * speedScale * deltaSeconds;

	m_camera.SetTransform(currentPosition, currentOrientation);
}