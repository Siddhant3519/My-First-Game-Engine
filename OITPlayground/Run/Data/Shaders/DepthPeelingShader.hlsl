...
Texture2D translucentPassDepthTexture	:	register(t1);
Texture2D opaqueDepthTexture			:	register(t2);
...
float4 PixelMain(v2p_t input) : SV_Target0
{
	...
	float maxDepth		=	opaqueDepthTexture[texelCoords].r;
	...
	// Discard the pixel if its before min depth
    float minDepth		=	translucentPassDepthTexture[texelCoords].r;
	bool  isDiscarded	=	input.position.z <= minDepth;
	if (isDiscarded)
    {
        discard;
    }
