...
{
	FMOD_VECTOR fmodSoundPosition = { -listenerPosition.y, listenerPosition.z, -listenerPosition.x };
	FMOD_VECTOR fmodSoundVelocity = { 0.f, 0.f, 0.f };
	FMOD_VECTOR fmodSoundForward = { -listenerForward.y, listenerForward.z, -listenerForward.x };
	FMOD_VECTOR fmodSoundUp = { -listenerUp.y, listenerUp.z, -listenerUp.x };
	
	m_fmodSystem->set3DListenerAttributes(listenerIndex, &fmodSoundPosition, &fmodSoundVelocity, &fmodSoundForward, &fmodSoundUp);
}
...
bool AudioSystem::IsPlaying(SoundPlaybackID soundPlaybackID)
{
	...
	// {
	// 	ERROR_RECOVERABLE("WARNING: attempt to check if sound is playing on missing sound playback ID!");
	// 	return false;
	// }
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, Vec3 const& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		unsigned int playbackMode = isLooped ? FMOD_LOOP_NORMAL : FMOD_LOOP_OFF;
		float frequency;
		FMOD_VECTOR fmodSoundPosition = { -soundPosition.y, soundPosition.z, -soundPosition.x };
		FMOD_VECTOR fmodSoundVelocity = { 0.f, 0.f, 0.f };
